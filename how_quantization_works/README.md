...
In this directory, I explore what is quantization in the context of Neural Networks. I specifically explore the derivation of the quantization and de-quantization equations.
...
- **Quantization-Aware Training** (QAT) incorporates the weight conversion process during the pre-training or fine-tuning stage, resulting in enhanced model performance. However, QAT is computationally expensive and demands representative training data. Quantization occurs during training to make the model robust to quantization errors, typically resulting in higher accuracy compared to post-training quantization, but at the high computational demands
...
For this projects seen here, I utilize **zerop-point** quantization. Zero-point quantization is a technique that transforms the original floating point number range into an 8-bit range (INT8).

Floating point numbers are floating point numbers are predominantly employed in deep learning due to their ability to represent a wide range of values with high precision. Typically, a floating point number uses n bits to store a numerical value. These _n_ bits are further partitioned into three distinct components:
...
8-bit integer range using 2's complement:
...
Zero-point quantization uses re-scaling and shifting to projec the FP range of tensors into the 8-bit range. Scaling and shifting are linear transformation operations.
...
$$
INT8 = scale \, factor \times FP + offset \qquad(1)
$$
...
$$
-128 = scale \, factor \times X_{min} + offset \qquad(2)
$$

$$
127 = scale \, factor \times X_{max} + offset \qquad(3)
$$

Subtracting equation (3) by equation (2), we can solve for **scaling factor**.

$$
127 - (-128) = scale \, factor \times (X_{max} - X_{min}) + (offest - offset)
$$

$$
255 = scale \, factor \times (X_{max} - X_{min}) \qquad(4)
$$

Rearranging equation (4), we can get the scaling factor:

$$
scale \, factor = \frac{255}{(X_{max} - X_{min})} \qquad(5)
$$
...
$$
offset = -scale \, factor \times X_{min} - 128
$$
...
$$
offset = -round(scale \, factor \times X_{min}) - 128 \qquad(7)
$$
...
Equation ensures that the offest will be an integer number. Now, we have to go back to equation (1) and round the rest of the right side
...
$$
INT8 = round(scale \, factor \times FP + offset) \qquad(9)
$$

because we rounded offset

$$
INT8 = round(scale \, factor \times FP) + offset \qquad(10)
$$
...
$$
\text{zero-point} = scale \, factor \times 0 + offset
$$
...
$$
\text{zero-point} = -round(scale \, factor \times X_{min}) - 128 \qquad(12)
$$
...
$$ scale \, factor = \frac{255}{max(X) - min(X)} $$
...
##### Summarizing Zero-Point Quantization and Dequantization Formulas

The Quantization Equation:

$$
INT8 = round(scale \, factor \times FP) + \text{zero-point} \qquad(14)
$$
